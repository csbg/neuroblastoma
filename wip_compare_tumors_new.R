...
samples_jansky <-
  read_csv("data_wip/samples_jansky.csv", comment = "#") %>% 
  transmute(
    sample,
...
singler_results_jansky <- 
  read_csv("data_wip/adrmed_class_wilcox_jansky.csv") %>% 
...
singler_results_nb <- 
  read_csv("data_wip/adrmed_class_wilcox_nb.csv") %>% 
...
projectils_results <-
  ...
  transmute(
    ...
    cell = str_sub(cell, 3),
    projectils_type = predicted_type
  )
...
singler_results <-
  ...
  inner_join(projectils_results, by = c("sample", "cell")) %>% 
  mutate(
    ...
    projectils_type =
      projectils_type %>% 
      factor(levels = levels(pruned_labels))
  )
...
plot_jansky_like <- function(type_col = pruned_labels) {
  singler_results_jansky %>%
    left_join(projectils_results, by = c("sample", "cell")) %>% 
    mutate(
      cell_type =
        {{type_col}} %>% 
        factor(levels = names(ADRMED_CELLS_COLORS)) %>% 
...
plot_jansky_like()
ggsave_default("comparison/jansky_3d_singler", height = 80, width = 200)

plot_jansky_like(projectils_type)
ggsave_default("comparison/jansky_3d_projectils", height = 80, width = 200)


## Cell type profile ----

plot_profile <- function(group_col = group, type_col = pruned_labels) {
  sample_order <- 
    singler_results %>% 
...
plot_profile()
ggsave_default("comparison/cell_types_group_singler", height = 420)

plot_profile(type_col = projectils_type)
ggsave_default("comparison/cell_types_group_projectils", height = 420)

plot_profile(group_col = group2)
ggsave_default("comparison/cell_types_group2_singler", height = 420)

plot_profile(group_col = group2, type_col = projectils_type)
ggsave_default("comparison/cell_types_group2_projectils", height = 420)



plot_agg_profile <- function(group_col = group, type_col = pruned_labels) {
  singler_results %>%
...
plot_agg_profile()
ggsave_default("comparison/profile_group_singler", height = 100)

plot_agg_profile(type_col = projectils_type)
ggsave_default("comparison/profile_group_projectils", height = 100)

plot_agg_profile(group_col = group2)
ggsave_default("comparison/profile_group2_singler", height = 100)

plot_agg_profile(group_col = group2, type_col = projectils_type)
ggsave_default("comparison/profile_group2_projectils", height = 100)
...
plot_score_heatmap <- function(sample, prop = 1) {
  ...
  ggsave_default(
    str_glue("comparison/heatmaps/{sample}"),
    plot = p, height = 100
  )
  p
}
